const GeradorQRCode = {
  ...
  init: function () {
    ...
    GeradorQRCode.selected.icon =
      $(".list-icons li.select").attr("value") || null;
    GeradorQRCode.selected.quality =
      parseInt($(".preview select[name='quality']").val()) || 0;
    GeradorQRCode.selected.iconsize =
      parseFloat($(".preview input[name='icon-size']").val()) || 0.7;
  ...
  uploadIcon: function (evt) {
    ...
    for (let i = 0; i < this.files.length; i++) {
      ...
      reader.onload = function (evt) {
        $(".list-icons li.upload").before(
          `<li value="other-${GeradorQRCode.uploadCount}"><img src="${evt.target.result}" width="50" height="50"></li>`
        );
        $(`.list-icons li[value="other-${GeradorQRCode.uploadCount}"]`).click(
          GeradorQRCode.selectIcon
        );
  ...
  selectType: function (evt) {
    ...
    if (GeradorQRCode.selected.type != null) {
      $(`.qr-type li[value='${GeradorQRCode.selected.type}']`).removeClass(
        "select"
      );
  ...
  selectIcon: function (evt) {
    ...
    if (GeradorQRCode.selected.icon != null) {
      $(`.list-icons li[value='${GeradorQRCode.selected.icon}']`).removeClass(
        "select"
      );
    }
  ...
  updateIconSize: function (evt) {
    $("#icon-size-box label").html(
      `Icone Tam. (${parseInt($(this).val() * 100)}%)`
    );
  ...
  createVCardOpts: function (opts) {
    ...
    if ($("input[name='data-vcar-site']").val().trim() != "") {
      vcard.add(
        vCard.Entry.URL,
        $("input[name='data-vcar-site']").val().trim()
      );
    }
    if ($("input[name='data-vcar-mail']").val().trim() != "") {
      vcard.add(
        vCard.Entry.EMAIL,
        $("input[name='data-vcar-mail']").val().trim(),
        "Trabalho"
      );
    }
    if ($("input[name='data-vcar-tel']").val().trim() != "") {
      vcard.add(
        vCard.Entry.PHONE,
        $("input[name='data-vcar-tel']").val().trim(),
        "Trabalho"
      );
    }
    if ($("input[name='data-vcar-cel']").val().trim() != "") {
      vcard.add(
        vCard.Entry.PHONE,
        $("input[name='data-vcar-cel']").val().trim(),
        "Celular"
      );
    }
    ...
    if ($("input[name='data-vcar-cname']").val().trim() != "") {
      vcard.add(
        vCard.Entry.ORGANIZATION,
        $("input[name='data-vcar-cname']").val().trim()
      );
    }
    if ($("input[name='data-vcar-cargo']").val().trim() != "") {
      vcard.add(
        vCard.Entry.TITLE,
        $("input[name='data-vcar-cargo']").val().trim()
      );
  ...
  setIconQR: function (opts) {
    if (
      GeradorQRCode.selected.icon == null ||
      GeradorQRCode.selected.icon == "none"
    ) {
      return;
    }

    opts.src = $(
      `.list-icons li[value='${GeradorQRCode.selected.icon}'] img`
    ).attr("src");
